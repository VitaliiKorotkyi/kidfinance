...
export default function HistoryCalendar({
  ...
  useEffect(() => {
    ...
    const from = new Date(month); from.setDate(1);
    ...
    supabase
      ...
      .then(({ data }) => setItems((data ?? []).map((x:any)=>({ date:x.date, points:x.points }))));
  }, [session, month]);
    ...
    <section className="rounded-2xl border p-4 bg-white">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button className="rounded border px-2 py-1" onClick={()=>setMonth(new Date(month.getFullYear(), month.getMonth()-1, 1))}>◀</button>
      ...
      <div className="grid grid-cols-7 gap-[2px]">
        {weeks.flat().map((cell, idx) => {
          ...
          const weak = pts < 0;
          return (
            <div key={idx} className={`h-16 rounded-md border flex flex-col items-center justify-center
