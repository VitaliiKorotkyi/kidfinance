...
type Exercise = {
  ...
  reps: number;
  points: number;
  tod: "morning" | "day" | "evening";
};
type WorkoutPlan = {
  exercises: Exercise[];
  autoIncrement: { enabled: boolean; perWeekReps: number };
  autoSync?: boolean;
...
const defaultWorkout: WorkoutPlan = {
  ...
  autoIncrement: { enabled: true, perWeekReps: 1 },
  autoSync: false,
  lastIncrementISO: new Date().toISOString().slice(0, 10),
};
...
const LS = {
  ...
  bad: "kf_bad",
  lang: "kf_lang",
  lastActive: "kf_last_active",
  tempPoints: "kf_temp_points",
  rules: "kf_rules",
};

/* ================ utils ================= */
const labelPart = (p: Task["timeOfDay"], l: Lang) =>
  p === "morning" ? t(l, "morning") : p === "day" ? t(l, "day") : t(l, "evening");
const todayISO = () => new Date().toISOString().slice(0, 10);
function weeksBetween(fromISO?: string, to = new Date()) {
  if (!fromISO) return 999;
  const d = new Date(fromISO);
  return Math.floor((to.getTime() - d.getTime()) / (7 * 24 * 60 * 60 * 1000));
}
...
function goldBar(w: number) {
  return (
    <div className="h-3 w-full rounded-full bg-yellow-100 overflow-hidden">
      <div
        className="h-3 rounded-full bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500"
...
export default function App() {
  ...
  const [lang, setLang] = useState<Lang>(
    (localStorage.getItem(LS.lang) as Lang) || "uk"
  );
  ...
  const [newTask, setNewTask] = useState({
    title: "",
    points: 10,
    timeOfDay: "day" as Task["timeOfDay"],
  });
  ...
  const [newEx, setNewEx] = useState<Exercise>({
    id: "new",
    nameKey: EX_KEYS[0],
    sets: 1,
    reps: 1,
  ...
  useEffect(() => {
    try {
      ...
      if (b) {
        ...
        setSpend(v.spend ?? 20);
        setSave(v.save ?? 50);
        setInvest(v.invest ?? 30);
      }
      if (h) setHistory(JSON.parse(h));
      if (w) setWorkout(JSON.parse(w));
      if (tg) setTargets(JSON.parse(tg));
      if (bd) setBad(JSON.parse(bd));
  ...
  useEffect(
    () => localStorage.setItem(LS.budget, JSON.stringify({ spend, save, invest })),
    [spend, save, invest]
  );
  ...
  useEffect(() => {
    if (!workout.autoIncrement.enabled || workout.autoIncrement.perWeekReps <= 0)
      return;
    ...
    setWorkout((p) => ({
      ...
      exercises: p.exercises.map((e) => ({
        ...e,
        reps: e.reps + weeks * p.autoIncrement.perWeekReps,
      })),
  ...
  const taskPoints = useMemo(
    () => tasks.filter((t) => t.done).reduce((s, t) => s + t.points, 0),
    [tasks]
  );
  const badPenalty = useMemo(
    () => bad.filter((b) => b.active).reduce((s, b) => s + b.penalty, 0),
    [bad]
  );
  const doneCount = useMemo(() => tasks.filter((t) => t.done).length, [tasks]);
  const strictPenalty =
    rules.strictEnabled && doneCount < rules.strictMinTasks
      ? rules.strictPenalty
      : 0;
  ...
  const yearPoints = useMemo(() => {
    const sum = history
      .filter((d) => d.dateISO.startsWith(yearKey))
  ...
  const effectiveRatePer100 =
    child.ratePer100 + (level - 1) * child.bonusPerLevel;
  const earningsToday = useMemo(
    () => (effectiveRatePer100 / 100) * netPoints,
    [effectiveRatePer100, netPoints]
  );
  ...
  function serializeState() {
    return {
      ...
      rules,
      targets,
      budget: { spend, save, invest },
      history, // локальную историю тоже храним, на всякий
    };
  }
  function applyState(s: any) {
    if (!s) return;
    setChild(s.child ?? defaultChild);
    setTasks(s.tasks ?? []);
    setWorkout(s.workout ?? defaultWorkout);
    setBad(s.bad ?? defaultBad);
    setRules(s.rules ?? defaultRules);
    setTargets(s.targets ?? { monthly: 2000, yearly: 24000 });
    if (s.budget) {
      setSpend(s.budget.spend ?? 20);
      setSave(s.budget.save ?? 50);
      setInvest(s.budget.invest ?? 30);
    }
    if (Array.isArray(s.history)) setHistory(s.history);
  }

  // после логина подтягиваем state из облака (или создаём запись)
  useEffect(() => {
    if (!session) return;
    (async () => {
      ...
      if (!data) {
        await supabase.from("app_state").insert({ user_id: uid, state: serializeState() });
      } else if (data.state) {
  ...
  const cloudSave = debounce(async () => {
    if (!session) return;
    const uid = session.user.id as string;
  ...
  useEffect(() => {
    cloudSave();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [session, child, tasks, workout, bad, rules, targets, spend, save, invest, history]);

  async function saveDayToCloud(dateISO: string, points: number) {
    if (!session) return;
    const earnings = Math.round(((child.ratePer100 + (level - 1) * child.bonusPerLevel) / 100) * points);
    await supabase.from("day_history").upsert({
      user_id: session.user.id,
      date: dateISO,
      points,
      earnings,
      breakdown: { badPenalty, strictPenalty, taskPoints },
      updated_at: new Date().toISOString(),
    });
  }
  ...
  // midnight auto-close
  useEffect(() => {
    const last = localStorage.getItem(LS.lastActive);
    const today = todayISO();
    if (last && last < today) {
      const temp = Number(localStorage.getItem(LS.tempPoints) || "0");
      saveDay(last, temp);
    }
  ...
  function toggleReminder(id: string) {
    setTasks((p) =>
      p.map((t) => (t.id === id ? { ...t, autoReminder: !t.autoReminder } : t))
    );
  }
  ...
  function rebalance(kind: "spend" | "save" | "invest", value: number) {
    ...
    a[rest[0]] = Math.round(remain * k1);
    a[rest[1]] = Math.round(remain * k2);
    const sum = a.spend + a.save + a.invest;
    if (sum !== 100) a[rest[1]] += 100 - sum;
    setSpend(a.spend);
    setSave(a.save);
    setInvest(a.invest);
  }
  ...
  const monthPoints = useMemo(() => {
    const sum = history
      .filter((d) => d.dateISO.startsWith(monthKey))
      .reduce((s, d) => s + d.points, 0);
    const inHist = history.some((d) => d.dateISO === todayISO());
    return sum + (inHist ? 0 : netPoints);
  ...
  const complexAutoPoints = useMemo(
    () => workout.exercises.reduce((s, e) => s + e.points, 0),
    [workout.exercises]
  );
  const [complexTod, setComplexTod] = useState<Task["timeOfDay"]>("morning");
  function addComplexToPlan() {
    const id = `W-${Date.now()}`;
    const children: TaskChild[] = workout.exercises.map((ex) => ({
      nameKey: ex.nameKey,
      sets: ex.sets,
      reps: ex.reps,
      points: ex.points,
    ...
    const task: Task = {
      id,
      labelKey: "complex",
      points: complexAutoPoints,
      timeOfDay: complexTod,
      autoReminder: true,
      done: false,
      children,
    };
    setTasks((p) => [...p, task]);
    setActiveTab(complexTod);
    setOpenComplex((o) => ({ ...o, [id]: true }));
  }

  const todayStr = new Intl.DateTimeFormat(
    lang === "uk" ? "uk-UA" : lang === "cs" ? "cs-CZ" : "en-US",
      ...
      <header className="sticky top-0 z-10 backdrop-blur bg-white/70 border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3 justify-between">
          <div className="flex items-center gap-3">
            ...
            {/* Можно скрыть BackupPanel, если больше не нужен */}
            <BackupPanel lang={lang} />
          </div>

          <div className="flex items-center gap-3">
            <AuthBar onSession={setSession} />
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <Field label={t(lang, "age")}>
              <input
                ...
                value={child.age}
                onChange={(e) =>
                  setChild({ ...child, age: parseInt(e.target.value || "0", 10) })
                }
              />
            </Field>
            <Field label={t(lang, "gender")}>
              <select
                ...
                value={child.gender}
                onChange={(e) =>
                  setChild({ ...child, gender: e.target.value as any })
                }
              >
                <option value="male">{t(lang, "boy")}</option>
                <option value="female">{t(lang, "girl")}</option>
                <option value="other">{t(lang, "other")}</option>
              </select>
            </Field>
            <Field label={t(lang, "temperament")}>
              <select
                className="w-full rounded-xl border px-3 py-2"
                value={child.temperament}
                onChange={(e) =>
                  setChild({ ...child, temperament: e.target.value as any })
                }
              >
                <option value="active">{t(lang, "active")}</option>
                <option value="calm">{t(lang, "calm")}</option>
                <option value="focused">{t(lang, "focused")}</option>
                <option value="mixed">{t(lang, "mixed")}</option>
              </select>
            </Field>
            <Field label={t(lang, "goal")} className="col-span-2">
              <select
                className="w-full rounded-xl border px-3 py-2"
                value={child.goal}
                onChange={(e) =>
                  setChild({ ...child, goal: e.target.value as any })
                }
              >
                <option value="balanced">{t(lang, "balanced")}</option>
                <option value="discipline">{t(lang, "discipline")}</option>
                <option value="health">{t(lang, "health")}</option>
                <option value="study">{t(lang, "study")}</option>
                <option value="creativity">{t(lang, "creativity")}</option>
              </select>
            </Field>

            <Field label={t(lang, "currencySymbol")}>
              <input
                className="w-full rounded-xl border px-3 py-2"
                value={child.currencySymbol}
                onChange={(e) =>
                  setChild({ ...child, currencySymbol: e.target.value })
                }
              />
            </Field>
            <Field label={t(lang, "ratePer100")}>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  min={0}
                  className="w-full rounded-xl border px-3 py-2"
                  value={child.ratePer100}
                  onChange={(e) =>
                    setChild({
                      ...child,
                      ratePer100: parseInt(e.target.value || "0", 10),
                    })
                  }
                />
                <span className="text-xs text-slate-500">{t(lang, "per100")}</span>
              </div>
            </Field>
            <Field label={t(lang, "bonusPerLevel")}>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  min={0}
                  className="w-full rounded-xl border px-3 py-2"
                  value={child.bonusPerLevel}
                  onChange={(e) =>
                    setChild({
                      ...child,
                      bonusPerLevel: parseInt(e.target.value || "0", 10),
                    })
                  }
                />
                <span className="text-xs text-slate-500">{t(lang, "per100")}</span>
        ...
        <Card className="lg:col-span-2">
          ...
          <div className="space-y-2">
            {byPart[activeTab].length === 0 && (
              <div className="text-sm text-slate-500">{t(lang, "none")}</div>
            )}
            {byPart[activeTab].map((tk) => {
              ...
              return (
                <div
                  key={tk.id}
                  className={`group rounded-2xl border bg-white p-3 ${
                    tk.done ? "opacity-60" : ""
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <label className="flex items-center gap-3">
                      ...
                      <div>
                        <div
                          className={`text-sm ${
                            tk.done ? "line-through text-slate-400" : "text-slate-900"
                          }`}
                        >
                          {tk.labelKey ? t(lang, tk.labelKey) : tk.title}
                        </div>
                        <div className="text-xs text-slate-500">
                          +{tk.points} {t(lang, "pointsShort")}
                        </div>
                      </div>
                    </label>
                    <div className="flex items-center gap-3">
                      ...
                      <label className="text-xs text-slate-600 flex items-center gap-2">
                        <input
                          ...
                          checked={tk.autoReminder}
                          onChange={() => toggleReminder(tk.id)}
                        />
                        {t(lang, "reminder")}
                      </label>
                      <button
                    ...
                    <div className="mt-2 rounded-xl border bg-slate-50 p-2 text-xs text-slate-700 space-y-1">
                      {tk.children!.map((c, i) => (
                        <div key={i} className="flex items-center justify-between">
                          <div>
                            {t(lang, c.nameKey)} — {c.sets}×{c.reps}{" "}
                            {t(lang, "repsShort")}
                          </div>
                          <div>
                            +{c.points} {t(lang, "pointsShort")}
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="mt-5 rounded-2xl border bg-white p-4">
            <div className="grid md:grid-cols-4 gap-3 items-end">
              <Field className="md:col-span-2" label={t(lang, "newTask")}>
                <input
                  className="w-full rounded-xl border px-3 py-2"
                  placeholder={t(lang, "placeholderNewTask")}
                  value={newTask.title}
                  onChange={(e) => setNewTask({ ...newTask, title: e.target.value })}
                />
              </Field>
              <Field label={t(lang, "points")}>
                <input
                  type="number"
                  min={1}
                  max={100}
              ...
              <Field label={t(lang, "time")}>
                <select
                  ...
                  onChange={(e) =>
                    setNewTask({ ...newTask, timeOfDay: e.target.value as any })
                  }
                >
                  <option value="morning">{t(lang, "morning")}</option>
                  <option value="day">{t(lang, "day")}</option>
                  <option value="evening">{t(lang, "evening")}</option>
                </select>
              </Field>
        ...
        <Card className="lg:col-span-2">
          ...
          <div className="grid md:grid-cols-4 gap-3">
            ...
            <StatTile
              label={t(lang, "monthProgress")}
              value={`${monthPoints}/${targets.monthly}`}
            />
            <StatTile
              label={t(lang, "yearProgress")}
              value={`${yearPoints}/${targets.yearly}`}
            />
          </div>
          <div className="rounded-xl border p-3 bg-white mt-3">
            ...
            <div className="mt-2 h-2 w-full rounded-full bg-slate-200 overflow-hidden">
              <div
                className="h-2 rounded-full bg-gradient-to-r from-slate-900 to-slate-600"
                style={{
                  width: `${Math.min(100, (Math.abs(netPoints) / 100) * 100)}%`,
                }}
              />
            </div>
          </div>
          <div className="grid md:grid-cols-2 gap-4 mt-5">
            <div className="rounded-2xl border p-4 bg-white">
              <div className="flex items-center justify-between text-sm mb-2">
                <div className="font-medium">{t(lang, "monthProgress")}</div>
                <div className="text-slate-600">
                  {monthPoints} / {targets.monthly}
                </div>
              </div>
              {goldBar(monthPct)}
            </div>
            <div className="rounded-2xl border p-4 bg-white">
              <div className="flex items-center justify-between text-sm mb-2">
                <div className="font-medium">{t(lang, "yearProgress")}</div>
                <div className="text-slate-600">
                  {yearPoints} / {targets.yearly}
                </div>
              </div>
              {goldBar(yearPct)}
              <div className="text-xs text-slate-600 mt-2">
                {t(lang, "level")}: <span className="font-semibold">Lv {level}</span>
              </div>
            </div>
          </div>
          <div className="grid md:grid-cols-3 gap-4 mt-6">
            ...
            <AllocationCard
              title={t(lang, "saving")}
              percent={save}
              onChange={(v) => rebalance("save", v)}
              amount={allocation.save}
              currency={currency}
            />
            <AllocationCard
              title={t(lang, "invest")}
              percent={invest}
              onChange={(v) => rebalance("invest", v)}
          ...
          <div className="mt-4 rounded-2xl border p-4 bg-white grid md:grid-cols-4 gap-3 items-end text-sm">
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                className="accent-slate-900"
                checked={rules.allowNegative}
                onChange={(e) =>
                  setRules((p) => ({ ...p, allowNegative: e.target.checked }))
                }
              />{" "}
              {t(lang, "allowNegative")}
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                className="accent-slate-900"
                checked={rules.strictEnabled}
                onChange={(e) =>
                  setRules((p) => ({ ...p, strictEnabled: e.target.checked }))
                }
              />{" "}
              {t(lang, "strictEnabled")}
            </label>
            <label>
              {t(lang, "strictMinTasks")}
              <input
                type="number"
                min={0}
                className="w-full rounded-xl border px-3 py-2 mt-1"
                value={rules.strictMinTasks}
                onChange={(e) =>
                  setRules((p) => ({
                    ...p,
                    strictMinTasks: Math.max(
                      0,
            ...
            <label>
              {t(lang, "strictPenalty")}
              <input
                type="number"
                min={0}
                className="w-full rounded-xl border px-3 py-2 mt-1"
                value={rules.strictPenalty}
                onChange={(e) =>
                  setRules((p) => ({
                    ...p,
                    strictPenalty: Math.max(0, parseInt(e.target.value || "0", 10)),
                  }))
                }
              />
            </label>
        ...
        <Card>
          ...
          <div className="text-sm text-slate-600 mb-3">
            ...
            <span className="font-medium">
              {workout.exercises.reduce((s, e) => s + e.sets * e.reps, 0)}{" "}
              {t(lang, "repsShort")} сумарно
            </span>
          </div>
          <div className="space-y-2">
              ...
              <div key={ex.id} className="rounded-xl border p-3 bg-white">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      {ex.sets} {t(lang, "sets").toLowerCase()} × {ex.reps}{" "}
                      {t(lang, "repsShort")} · +{ex.points} {t(lang, "pointsShort")} ·{" "}
                      {labelPart(ex.tod, lang)}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={() =>
                        setWorkout((p) => ({
                          ...p,
                          exercises: p.exercises.map((e) =>
                            e.id === ex.id ? { ...e, reps: Math.max(1, e.reps - 1) } : e
                          ),
                        }))
                      }
                    >
                    ...
                    <button
                      ...
                      onClick={() =>
                        setWorkout((p) => ({
                          ...p,
                          exercises: p.exercises.filter((e) => e.id !== ex.id),
                        }))
                      }
                    >
                      {t(lang, "delete")}
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <Field label={t(lang, "exercise")}>
                <select
                  ...
                  onChange={(e) =>
                    setNewEx((s) => ({
                      ...s,
                      nameKey: e.target.value as (typeof EX_KEYS)[number],
                    }))
                  }
                >
                  {EX_KEYS.map((k) => (
              ...
              <Field label={t(lang, "sets")}>
                <input
                  ...
                  onChange={(e) =>
                    setNewEx((s) => ({
                      ...s,
                      sets: Math.max(1, parseInt(e.target.value || "1", 10)),
                    }))
                  }
                />
              </Field>
              <Field label={t(lang, "reps")}>
                <input
                  type="number"
                  min={1}
                  className="w-full rounded-xl border px-3 py-2"
                  ...
                  onChange={(e) =>
                    setNewEx((s) => ({
                      ...s,
                      reps: Math.max(1, parseInt(e.target.value || "1", 10)),
                    }))
                  }
                />
              </Field>
              <Field label={`${t(lang, "points")} (${t(lang, "pointsShort")})`}>
                <input
                  type="number"
                  min={0}
                  className="w-full rounded-xl border px-3 py-2"
                  ...
                  onChange={(e) =>
                    setNewEx((s) => ({
                      ...s,
                      points: Math.max(0, parseInt(e.target.value || "0", 10)),
                    }))
                  }
                />
              </Field>
              <Field label={t(lang, "time")}>
                <select
                  className="w-full rounded-xl border px-3 py-2"
                  value={newEx.tod}
                  onChange={(e) => setNewEx((s) => ({ ...s, tod: e.target.value as any }))}
                >
                  <option value="morning">{t(lang, "morning")}</option>
                  <option value="day">{t(lang, "day")}</option>
                  <option value="evening">{t(lang, "evening")}</option>
              ...
              <div className="col-span-2 text-right">
                <button
                  ...
                  onClick={() => {
                    setWorkout((p) => ({
                      ...p,
                      exercises: [...p.exercises, { ...newEx, id: crypto.randomUUID() }],
                    }));
                    setNewEx({
                      id: "new",
                      nameKey: EX_KEYS[0],
          ...
          <div className="mt-4 rounded-2xl border p-3 bg-white">
            <div className="grid grid-cols-2 gap-2 items-end">
              <Field label={t(lang, "complexTod")}>
                <select
                  className="w-full rounded-xl border px-3 py-2"
                  value={complexTod}
                  onChange={(e) => setComplexTod(e.target.value as any)}
                >
                  <option value="morning">{t(lang, "morning")}</option>
                  <option value="day">{t(lang, "day")}</option>
                  <option value="evening">{t(lang, "evening")}</option>
              ...
              <div className="text-right text-sm text-slate-600">
                Auto: <span className="font-semibold">+{complexAutoPoints}</span>{" "}
                {t(lang, "pointsShort")}
              </div>
            </div>
            <div className="text-right mt-3">
              <button className="btn-primary" onClick={addComplexToPlan}>
                {t(lang, "addComplexToPlan")}
              </button>
            </div>
          </div>
          ...
          <div className="mt-3 rounded-2xl bg-slate-50 border p-3 text-sm">
            ...
            <ul className="list-disc pl-5 space-y-1">
              {workout.exercises.map((ex) => {
                ...
                return (
                  <li key={ex.id}>
                    {t(lang, ex.nameKey)}: {min}–{max} {t(lang, "repsShort")}/трен.
                  </li>
                );
              })}
            </ul>
            {workoutWarnings.length > 0 && (
              <div className="mt-2 rounded-xl border border-amber-300 bg-amber-50 p-2 text-amber-800">
                {workoutWarnings.map((m, i) => (
                  <div key={i}>{m}</div>
                ))}
              </div>
            )}
        ...
        <Card>
            ...
            {bad.map((bi) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBad((p) =>
                      p.map((x) => (x.id === bi.id ? { ...x, active: e.target.checked } : x))
                    )
                  }
                />
              </label>
          ...
          <div className="mt-3 rounded-xl border p-3 bg-white grid grid-cols-3 gap-2 items-end text-sm">
            <Field label={t(lang, "addCustomBad")} className="col-span-2">
              <input
                className="w-full rounded-xl border px-3 py-2"
                placeholder={t(lang, "placeholderCustomPenalty")}
                value={customBad.title}
                onChange={(e) => setCustomBad({ ...customBad, title: e.target.value })}
              />
            </Field>
            <Field label="−pts">
              <input
                ...
                min={1}
                className="w-full rounded-xl border px-3 py-2"
                value={customBad.penalty}
                onChange={(e) =>
                  setCustomBad({
                    ...customBad,
                    penalty: Math.max(1, parseInt(e.target.value || "1", 10)),
                  })
                }
              />
            </Field>
            <div className="col-span-3 text-right">
              <button
                className="btn-primary"
                onClick={() => {
                  if (!customBad.title.trim()) return;
                  setBad((p) => [
                    ...p,
                    {
                      id: crypto.randomUUID(),
                      title: customBad.title.trim(),
                      penalty: customBad.penalty,
                      active: true,
                    },
                  ]);
                  setCustomBad({ title: "", penalty: 5 });
                }}
              >
                {t(lang, "add")}
              </button>
        ...
        <Card>
          ...
          <div className="rounded-2xl border bg-white divide-y">
            {tasks
              .filter((tk) => tk.autoReminder)
              .map((tk) => (
                <div key={tk.id} className="p-3 text-sm hover:bg-slate-50">
                  <span className="mr-2 text-slate-500">
                    {labelPart(tk.timeOfDay, lang)}:
                  </span>
                  {tk.labelKey ? t(lang, tk.labelKey) : tk.title}
                </div>
              ))}
            {tasks.filter((tk) => tk.autoReminder).length === 0 && (
              <div className="p-3 text-sm text-slate-500">{t(lang, "none")}</div>
            )}
          </div>
        </Card>

        {/* Calendar (cloud history) */}
        {session && (
          <div className="lg:col-span-3">
            <HistoryCalendar session={session} />
          </div>
        )}
      </div>
...
function Card({
  ...
  return (
    <section
      className={`rounded-2xl border bg-white p-5 shadow-sm hover:shadow transition ${className}`}
    >
      {children}
    </section>
  );
  ...
  return (
    <div className="rounded-2xl border p-4 bg-white">
      ...
      <input
        ...
        className="w-full h-2 rounded bg-slate-200 accent-slate-900"
      />
